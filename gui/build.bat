...
rem  产物：dist\github-desktop-zh-1.0.1.exe  （双击即用，不依赖任何运行环境）
...
echo [3/4] 编译为单文件 EXE（GUI 版，隐藏控制台；体积优化见仓库根 github-desktop-zh-1.0.1.spec）...
python -m PyInstaller --clean --noconfirm --distpath "dist" --workpath "build" "..\github-desktop-zh-1.0.1.spec"

rem 如需同时打包命令行版 EXE，取消下行注释：
rem python -m PyInstaller --onefile --name github-desktop-zh-1.0.1-cli --add-data "..\dictionaries;dictionaries" cli.py

echo [4/4] 完成。产物位于：dist\github-desktop-zh-1.0.1.exe
if exist "dist\github-desktop-zh-1.0.1.exe" (
    echo 文件大小：
    for %%F in ("dist\github-desktop-zh-1.0.1.exe") do echo   %%~zF 字节
)
echo.
echo 提示：
